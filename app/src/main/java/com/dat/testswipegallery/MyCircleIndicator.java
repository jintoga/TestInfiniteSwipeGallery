...
public class MyCircleIndicator extends LinearLayout {
    ...
    public void setViewPager(LoopRecyclerViewPager viewPager) {
        mViewpager = viewPager;
        if (mViewpager != null && mViewpager.getAdapter() != null) {
            mLastPosition = -1;
            createIndicators();
            mViewpager.removeOnPageChangedListener(mInternalRecyclerViewPageChangeListener);
            mViewpager.addOnPageChangedListener(mInternalRecyclerViewPageChangeListener);
            mInternalRecyclerViewPageChangeListener.OnPageChanged(-1,
                mViewpager.getActualCurrentPosition());
        }
    }

    private final RecyclerViewPager.OnPageChangedListener mInternalRecyclerViewPageChangeListener =
        new RecyclerViewPager.OnPageChangedListener() {
            ...
            public void OnPageChanged(int oldPosition, int newPosition) {
                ...
                View currentIndicator = getChildAt(transformToActualPosition(oldPosition));
                if (mLastPosition >= 0 && currentIndicator != null) {
                    currentIndicator.setBackgroundResource(mIndicatorUnselectedBackgroundResId);
                    mAnimatorIn.setTarget(currentIndicator);
                    Log.d("mAnimatorIn", "start");
                    mAnimatorIn.start();
                }

                View selectedIndicator = getChildAt(transformToActualPosition(newPosition));
                if (selectedIndicator != null) {
                    selectedIndicator.setBackgroundResource(mIndicatorBackgroundResId);
                    mAnimatorOut.setTarget(selectedIndicator);
                    Log.d("mAnimatorOut", "start");
                    mAnimatorOut.start();
                }
                mLastPosition = transformToActualPosition(newPosition);
            }
        };
    ...
    private void createIndicators() {
        ...
        int currentItem = mViewpager.getActualCurrentPosition();
    ...
    public int transformToActualPosition(int position) {
        return position % mViewpager.getAdapter().getItemCount();
    }
}